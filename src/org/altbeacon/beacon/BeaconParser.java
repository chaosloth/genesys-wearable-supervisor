...
public class BeaconParser {
    ...
    protected Beacon fromScanData(byte[] scanData, int rssi, BluetoothDevice device, Beacon beacon) {
        ...
        byte[] tmpHeader = new byte[typeCodeBytes.length];
        ...
        while (startByte <= 5) {
            ...
            System.arraycopy(scanData, startByte+mMatchingBeaconTypeCodeStartOffset, tmpHeader, 0, typeCodeBytes.length);
            BeaconManager.logDebug(TAG, "**CJC: Offset[" + startByte + "] Type code bytes = " + byteArrayToString(typeCodeBytes) + " != Header " + byteArrayToString(tmpHeader));
